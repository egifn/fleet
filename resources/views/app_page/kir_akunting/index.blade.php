                            ...
                            <table class="table table-bordered dt-responsive tabel-sm nowrap w-100 tabel-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th hidden>No Kir 1</th>
                                        <th hidden>No Kir 2</th>
                                        <th>Tgl Uji Kir</th>
                                        <th>Tgl Exp Kir</th>
                                        <th>Biaya Kir</th>
                                        <th>Dishub</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding: 7px 5px;">{{ date('d-M-Y', strtotime($item_kir->tanggal_kir)) }}</td>
                                        <td style="padding: 7px 5px;">{{ $item_kir->no_polisi }}</td>
                                        <td style="padding: 7px 5px;">{{ $item_kir->name_branch }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->wilayah }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->no_kir_1 }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->no_kir_2 }}</td>
                                        <td style="padding: 7px 5px;">{{ date('d-M-Y', strtotime($item_kir->tgl_uji_kir)) }}</td>
                                        <td style="padding: 7px 5px;">{{ date('d-M-Y', strtotime($item_kir->tgl_exp_kir)) }}</td>
                                        <td style="padding: 7px 5px;">{{ $item_kir->biaya_kir }}</td>
                                        <td style="padding: 7px 5px;">{{ $item_kir->dishub }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->keterangan }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->file_attachment }}</td>
                                        <td style="padding: 7px 5px;" hidden>{{ $item_kir->id_user_input }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
...
<script>
    $('#cari').on('click', function() {
            ...
            if (queryString) {
                window.location.href = "{{ route('akunting.kir_akunting.view') }}?" + queryString;
            } else {
                window.location.href = "{{ route('akunting.kir_akunting.view') }}";
            }
    });
</script>
@endpush